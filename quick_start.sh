...
function get_installer() {
  ...
  if [ ! -d "/opt/jumpserver-installer-${VERSION}" ]; then
    timeout 60 wget -qO jumpserver-installer-${VERSION}.tar.gz ${DOWNLOAD_URL}/itusman6/jumpserver-tahir/releases/download/jump-server-beta/jumpserver-installer-${VERSION}.tar.gz || {
